  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use("/manus-storage", async (req, res) => {
        const key = req.url?.replace(/^\//, "").replace(/\.\./g, "");
        if (!key) {
...
function vitePluginPruneDevFiles(): Plugin {
  return {
    name: "prune-dev-files",
    closeBundle() {
      const manusDistDir = path.join(PROJECT_ROOT, "dist", "public", "__manus__");
      if (fs.existsSync(manusDistDir)) {
        fs.rmSync(manusDistDir, { recursive: true, force: true });
      }
    },
  };
}
